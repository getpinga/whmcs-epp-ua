...
function ua_manageDNSSECDSRecords($params = array())
{
    _ua_log(__FUNCTION__, $params);
    $return = array();
    try {
...
<epp xmlns="urn:ietf:params:xml:ns:epp-1.0"
  ...
  <command>
    ...
    <clTRID>{{ clTRID }}</clTRID>
        ...
        $secDNSdsData = array();
        if ($r->response->extension && $r->response->extension->children('http://hostmaster.ua/epp/secDNS-1.1')->infData) {
            $DSRecords = 'YES';
            $i = 0;
            $r = $r->response->extension->children('http://hostmaster.ua/epp/secDNS-1.1')->infData;
...
function ua_ClientAreaCustomButtonArray()
{
    $buttonarray = array(
